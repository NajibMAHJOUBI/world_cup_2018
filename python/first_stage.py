...
class FirstRound:
    def __init__(self, spark_session, classification_model, confederation, round_start_date=None, round_end_date=None):
...
#        self.save_matches_next_stage(dic_first_by_group, dic_second_by_group)
    ...
    def get_matches_results(self, start_date, end_date):
        ...
        if (start_date is None):
            return FeaturizationData(self.spark, None).get_qualifying_results_data(self.confederation)
        else:
            return (FeaturizationData(self.spark, None).get_qualifying_results_data(self.confederation)
