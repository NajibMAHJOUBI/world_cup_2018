...
#        self.stacking_transform(new_data).show(10)
    ...
    def load_all_data(self):
        dic_data = {}
#        print(os.path.join(self.path, self.classifier_available[0]))
        dic_data[self.classifier_available[0]] =  (self.spark.read.csv(os.path.join(self.path, self.classifier_available[0]), sep=",", header=True, schema=self.get_schema())
                                                   .withColumnRenamed("prediction", self.classifier_available[0]))

        for classifier in self.classifier_available[1:]:
            dic_data[classifier] = (self.spark.read.csv(os.path.join(self.path, classifier), sep=",", header=True, schema=self.get_schema())
                                    .drop("label")
