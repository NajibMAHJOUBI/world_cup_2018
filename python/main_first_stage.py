...
featurization = FeaturizationData(spark, ["WCF"], list_date=dic_date["2nd_stage"])
featurization.run()
...
for model in classification_model:
    ...
    first_round = FirstRound(spark, model, False, data,
                             "../test/classification_model", "../test/prediction/second_stage")
    first_round.run()
